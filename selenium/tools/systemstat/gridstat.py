...
class GridStat(systemstat.SystemStat):
    ...
    def is_ready(self):
        ...
        if response.ok:
            ...
            self.logger.info('{} of {} nodes are attached'.format(
                slotCounts['total'], self._nodes))
            ...
            if slotCounts['total'] == self._nodes:
                ...
                self.logger.info('{} of {} nodes are ready'.format(
                    slotCounts['free'], self._nodes))
                ...
                if slotCounts['free'] == self._nodes:
                    ...
                    self.logger.info('waiting on {} node(s) to be ready'.format(
                        self._nodes-slotCounts['free']))

            else:
                # nodes are not attached yet
                self.logger.info('waiting on {} node(s) to attach'.format(
                    self._nodes-slotCounts['total']))

        else:
            # response was not "ok", log error details
            self.logger.info(
                'hub responded at "{}" with error:\n{}\n{}\n{}'.format(
                    grid_api_hub_url, response.status_code, response.headers,
                    response.text))
